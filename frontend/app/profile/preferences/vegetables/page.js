...
export default function EditVegetablePreferencesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE_URL}/vegetables`).then((res) => res.json()),
      fetch(`${API_BASE_URL}/users/${storedUserId}/food-preferences`).then((res) => res.json()),
    ])
      .then(([allVegetables, foodPreferences]) => {
        setVegetables(allVegetables);
        const savedNames = foodPreferences.vegetables.map((v) => v.name);
        setSelected(
          allVegetables.filter((v) => savedNames.includes(v.name)).map((v) => v.id)
        );
        setLoading(false);
      })
      .catch(() => {
        setError("Could not load vegetable preferences.");
        setLoading(false);
      });
  }, []);
  ...
  async function handleContinue() {
    ...
    try {
      ...
      router.push("/profile/preferences/success");
    } catch {
      setError("Could not save vegetable preferences.");
      setSaving(false);
    }
  ...
  return (
    <main className="min-h-screen w-full bg-background px-4 py-8">
      <div className="max-w-2xl mx-auto">
        <OnboardingHeader backHref="/profile/preferences/proteins" step={2} totalSteps={3} />
